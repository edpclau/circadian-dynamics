...
#' @return A named list of data.frames containing the output of [find_gaps()], [makes_time_windows()], and [smooth_detrend_by_windows()] for each measurement value.
#'
#' @export
#'
#' @examples
...
#'
process_timeseries <- function(df = NULL, sampling_rate = NULL, window_size_in_days = 3, window_step_in_days = 1, smooth_data = TRUE, detrend_data = FALSE,
         datetime = NULL, values = NULL) {
...
windowed_data <- dplyr::bind_cols(datetime = windowed_data$datetime,
                                  smooth_detrend_by_windows(dplyr::select(windowed_data, -datetime) , smooth_data = smooth_data, detrend_data = detrend_data))
