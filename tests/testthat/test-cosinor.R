test_that("cosinor recovers amplitude/mesor with non-unit bin size (30 min)", {
  ...
  res <- analyze_cosinor(df, sampling_rate = "30 minutes", period = 24)
...
test_that("amplitude_se scales linearly with signal magnitude (delta method / amplitude)", {
  ...
  r1 <- analyze_cosinor(df1, sampling_rate = "1 hour", period = 24)
  r2 <- analyze_cosinor(df2, sampling_rate = "1 hour", period = 24)
